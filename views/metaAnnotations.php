<?php if($post->ID > 0): ?>
<style type='text/css'>
	#ANNOtype_metaAnnotations { border: 1px solid #9badcb; }
	#annotateButton { display: block; width: 244px; text-align: center; }
	#annotation { list-style: none; margin: 0; padding: 0; }
	#annotation li { background: #eef5fb; font-style: italic; margin: 0; padding: 5px; position: relative; }
	.deleteAnnotation { background: url('<?php echo UBD_URL; ?>/public/images/delete.png'); width: 9px; height: 9px; display: none; text-indent: -100000px; position: absolute; top: 0; right: 0; }
	#simplemodal-overlay { background-color: #FFFFFF; }
	#simplemodal-container { background-color: #FFFFFF; border: 8px solid #DFDFDF; padding: 12px; }
</style>
...
jQuery(document).ready(function($) {
	...
	$('#annotation li:nth-child(even)').css('background-color', '#fff');
	
	$('#annotateButton').click(function() {
		var selectedText = '';
		if (tinyMCE.activeEditor != null && tinyMCE.activeEditor.isHidden() == false) {
			selectedText = tinyMCE.get('content').selection.getContent();
		} else {
			selectedText = $('#content').getSelection().text;
		}
		...
		if(selectedText) {
			...
			$('#addAnnotationButton').click(function() {
				var annoText = $(this).parents('#addAnnotation').find('textarea').val();
				var annoAction = $(this).parents('#addAnnotation').find('input[name=addAnnotationAction]').val();
				var annoType = $(this).parents('#addAnnotation').find('input[name=addAnnotationType]').val();
				var annoPostID = $(this).parents('#addAnnotation').find('input[name=addAnnotationPostID]').val();
				var annoUserID = $(this).parents('#addAnnotation').find('input[name=addAnnotationUserID]').val();
				var annoSL = selectedText;
				
				if(annoText) {
					$.post(
						ajaxurl,
						{selectedText: annoSL, action: annoAction, text: annoText, type: annoType, postID: annoPostID, userID: annoUserID},
						function(data) {
							if (tinyMCE.activeEditor != null && tinyMCE.activeEditor.isHidden() == false) {
								tinyMCE.execCommand('mceReplaceContent', false, '<span class="annotation" id="annotationID_'+data+'">'+selectedText+'</span>')
							} else {
								wrapText('content', '<span class="annotation" id="annotationID_'+data+'">', '</span>');
							}

							var newSup = 1;

							if($('#annotation li:last sup').text() != '') {
								var newSup = parseInt($('#annotation li:last sup').text()) + 1;
							}
							
							$('#annotation').append('<li>a<sup>'+newSup+'</sup> - '+annoSL+' <a href="#" class="deleteAnnotation" id="deleteAnnotationID_'+data+'">Delete</a></li>')
					...
					alert('Enter some text to annotate.');
				}
				
				return false;
			});
		} else {
			alert('Select text to annotate.');
		}
	...
	$('.deleteAnnotation').live('click', function() {
		...
		$.post(
			ajaxurl,
			{action: 'ANNOtype_deleteAnnotation', annotationID: ID},
			function(data) {
				if (tinyMCE.activeEditor != null && tinyMCE.activeEditor.isHidden() == false) {
					tinyMCE.activeEditor.dom.setOuterHTML('annotationID_'+data, $(tinyMCE.activeEditor.dom.get('annotationID_'+data)).text());		
				} else {
					var text = $('#content').val();
					var regex = new RegExp('<span class="annotation" id="annotationID_'+data+'">(.+)<\/span>')
	...
	$('#annotation li').live('mouseover', function() {
		$(this).find('.deleteAnnotation').show();
	}).live('mouseout', function() { 
		$(this).find('.deleteAnnotation').hide();
	});
});
</script>
<a href='#' class='button' id='annotateButton'>Click Here to Highlight</a>
<p>You MUST <strong>HIGHLIGHT TEXT</strong> to annotate.</p>
<ul id='annotation'>
<?php if($this->getAnnotationsByID($post->ID)): ?>
	<?php $count = 1; ?>
	<?php foreach($this->getAnnotationsByID($post->ID) as $annotation): ?>
	<li>
		a<sup><?php echo $count; ?></sup> - <?php echo $annotation->text; ?>
		<a href='#' class='deleteAnnotation' id='deleteAnnotationID_<?php echo $annotation->ID; ?>'>Delete</a>
	</li>
	<?php $count++; endforeach; ?>
<?php endif; ?>
</ul>
<div id='addAnnotation' style='display: none;'>
	<p>Write your annotation:</p>
	<p><textarea name='annotation' cols='50' rows='5'></textarea></p>
	<input type='hidden' name='addAnnotationAction' value='ANNOtype_addAnnotation' />
	<input type='hidden' name='addAnnotationType' value='annotation' />
	<input type='hidden' name='addAnnotationPostID' value='<?php echo $post->ID; ?>' />
	<input type='hidden' name='addAnnotationUserID' value='<?php echo $current_user->ID; ?>' />
	<p><input class='button' type='submit' value='Annotate' id='addAnnotationButton' /></p>
</div>
<?php else: ?>
<p>Save or publish the post to enable ANNOtype features.</p>
<?php endif; ?>